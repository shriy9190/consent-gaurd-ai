...
def _extract_registered_domain(host):
    if ":" in host:
        host = host.split(":")[0]
    parts = host.split(".")
    if len(parts) > 2:
        double_tlds = {"co.uk", "com.br", "co.jp", "com.tr", "co.in", "org.uk", "gov.uk"}
        tld_candidate = ".".join(parts[-2:])
        if tld_candidate in double_tlds and len(parts) > 3:
            return ".".join(parts[-3:])
        else:
            return ".".join(parts[-2:])
    return host
...
def _normalize_url(url):
    ...
    return _extract_registered_domain(url)
...
# ── Main functions (names match what app.py expects) ─────────────────────────

def get_cached_result(url):          # ✅ was check_cache — renamed to match app.py
    try:
...
def save_result(url, cleaned_text, analysis):   # ✅ was save_cache — renamed + accepts cleaned_text param
    try:
        collection = _get_collection()
        doc_id = url_to_id(url)
        cleaned_url = _normalize_url(url)
